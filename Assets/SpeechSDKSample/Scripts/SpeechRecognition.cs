...
public class SpeechRecognition : MonoBehaviour
{
    ...
    //[Tooltip("Source language for translation (if enabled).")]
    //public Dropdown SourceLang;
    ...
    // e.g. fr-fr, es-es, etc.
    string fromLanguage = "hi-IN";
    //string fromLanguage;
    //Debug.Log(Languages2.value);
    //string fromLanguage = ExtractLanguageCode(Languages2.options[Languages2.value].text);
    //string fromLanguage = Languages2.captionText.text;
    ...
    {
        Debug.Log(Languages1.value);
        Debug.Log(Languages1.options[Languages1.value].text);
    }
...
#region Speech Translation event handlers
    ...
    private void RecognizingTranslationHandler(object sender, TranslationRecognitionEventArgs e)
    {
        ...
        if (e.Result.Reason == ResultReason.TranslatingSpeech)
        {
            ...
            lock (threadLocker)
            {
                recognizedString = $"RECOGNIZED HYPOTHESIS ({ExtractLanguageCode(fromLanguage)}): {Environment.NewLine}{e.Result.Text}";
